...
//			String customerName = lib.GetCellToString.getCellValue(file.getSheet(0).getRow(i).getCell(3));
//			WebElement eCustomerName = lib.getElement.getElementById(driver, "field_Customer");
//			eCustomerName.sendKeys(customerName);
			
			List<WebElement> eRowInPage = driver.findElements(By.xpath(".//*[contains(@class,'form-group')]"));
			WebElement eRowCustomerName = eRowInPage.get(2); //
			// WebElement ePopupCustomerName =
			// eRowCustomerName.findElement(By.tagName("span"));
			...
			try {
				...
				while (isFind == false) {
					...
					String customerName = lib.GetCellToString.getCellValue(file.getSheet(0).getRow(i).getCell(3));
					WebElement foundCustomerName = getRow(listCustomerName, customerName);

					if (foundCustomerName != null) {
						WebElement saveButton = popup
								.findElement(By.xpath(".//*[contains(@jhitranslate,'entity.action.save')]"));
						foundCustomerName.click();
						Thread.sleep(1000);
						saveButton.click();
						System.out.print("hahahhaha   " + saveButton);
						Thread.sleep(1000);
						isFind = true;
						break;

					} else {
						System.out.println("STT " + customerName + " : not found in this page. Next page...!");
						...
						if (tabIndex == null) {
							...
							System.out.println("Row: " + i + " UserID = " + customerName
			...
			/*--------------Insert Field 7:  Development staff  --------------*/
			String developmentStaff = lib.GetCellToString.getCellValue(file.getSheet(0).getRow(i).getCell(7));
			...
			String contractName = lib.GetCellToString.getCellValue(file.getSheet(0).getRow(i).getCell(17));
			...
			String notificationExpect = file.getSheet(0).getRow(i).getCell(18).getStringCellValue();
	...
	private String getUserName(WebElement element) {
		...
		WebElement eUserName = list.get(1);
